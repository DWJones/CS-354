...
\newcommand{\Lit}{\mathcal{LIT}}
\newcommand{\Cl}{\mathcal{CL}}
\newcommand{\Cls}{\mathcal{CLS}}
...
Boolean logic is referred to as two-valued logic. This is because two values are used to determine if something is true of false. \texttt{True} is represented as $1$ and \texttt{false} is $0$. The connectives are $\lor, \land, \neg, \rightarrow, \leftrightarrow$.

XXX

The semantics for the connectives are shown in truth tables. A \emph{truth table} computes the logical values that are given to their corresponding value over a given operation. An example of this would be the negation of $p$ which would be $\neg p$. 
\begin{center}
\begin{tabular}{|c|c|c|}
\hline
$p$ & $\neg p$\\ \hline
